...
ToAscii.argtypes = (wintypes.UINT, wintypes.UINT, POINTER(keyboard_state), wintypes.LPWORD)
...
def hook_function(nCode, wParam, lParam):
    ...
    if wParam == WM_KEYDOWN:
        ...
        GetKeyState(WM_RETURN)
        GetKeyboardState(byref(state))
        
        buf = (c_ushort * 1)()
        n = ToAscii(keyboard.vkCode, keyboard.scanCode, state, buf)
