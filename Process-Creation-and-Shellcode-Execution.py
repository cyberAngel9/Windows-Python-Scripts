...
class STARTUPINFO(Structure):
    _fields_ = [
        ("cb", wintypes.DWORD),
        ("lpReserved", wintypes.LPWSTR),  # Updated LPCTSTR to LPWSTR
        ("lpDesktop", wintypes.LPWSTR),  # Updated LPSTR to LPWSTR
        ("lpTitle", wintypes.LPWSTR),    # Updated LPSTR to LPWSTR
        ("dwx", wintypes.DWORD),
        ("dwy", wintypes.DWORD),
...
CreateProcessA.argtypes = (
    wintypes.LPCSTR, wintypes.LPSTR, LPSECURITY_ATTRIBUTES,
    LPSECURITY_ATTRIBUTES, wintypes.BOOL, wintypes.DWORD,
    wintypes.LPVOID, wintypes.LPCSTR, POINTER(STARTUPINFO),
...
print("Startted process => Handle:{}, PID:{}, TID:{}".format(h_process, pid, thread_id))
